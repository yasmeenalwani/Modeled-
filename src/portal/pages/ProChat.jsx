...
const MOCK_PRO_ID = 'mock-pro-1';
const MOCK_PRO_NAME = 'Sarah M.';
...
// Session context data keyed by conversation ID
const SESSION_CONTEXT = {
  'conv-pro-model-1': {
    service: 'Haircut',
    modelName: 'Seraphina L.',
    date: 'Tomorrow',
    time: '10:00 AM',
    location: 'Luxe Studio, 123 Beauty St, New York',
    status: 'confirmed',
    fee: '$65',
    matchScore: 94,
  },
...
function ConversationItem({ conv, messages, selected, onClick, currentUserId }) {
  ...
  const avatarLetter = isProModel ? 'S' : 'M';
  const name = isProModel ? 'Seraphina L.' : 'Modeled Support';
      ...
      <div style={{ position: 'relative', flexShrink: 0 }}>
        <div style={{
          ...
          background: isProModel
            ? 'linear-gradient(135deg, #8B1E3F, #C4607A)'
    ...
    <div style={{
      ...
      <div style={{ flex: 1, minWidth: '160px' }}>
        ...
        <div style={{ fontSize: '0.95rem', fontWeight: '700', color: '#4A2A1A', fontFamily: '"Alike", serif' }}>
          {ctx.service} with {ctx.modelName}
        </div>
      ...
      <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
        ...
        <span style={{ background: 'rgba(139,30,63,0.1)', color: '#8B1E3F', fontSize: '0.75rem', fontWeight: '700', padding: '3px 10px', borderRadius: '999px' }}>
          {ctx.matchScore}% match
        </span>
        <span style={{ background: 'rgba(74,42,26,0.07)', color: '#4A2A1A', fontSize: '0.75rem', fontWeight: '600', padding: '3px 10px', borderRadius: '999px' }}>
          Pro Fee: {ctx.fee}
        </span>
      </div>
    </div>
    ...
    <div style={{
      ...
      {!isMine && (
        <div style={{
          ...
          height: 32,
          borderRadius: '50%',
          background: 'linear-gradient(135deg, #8B1E3F, #C4607A)',
...
export default function ProChat() {
  ...
  const currentUserId = shouldUseMockData() ? MOCK_PRO_ID : (user?.userId || MOCK_PRO_ID);
  const currentUserName = MOCK_PRO_NAME;
  ...
  const handleSend = () => {
    ...
    // Simulate typing response for pro-model channel
    ...
    <div style={{
      ...
      {selectedConv ? (
        <div style={{
          ...
          <div style={{
            ...
            <div>
              <div style={{ fontWeight: '700', color: '#4A2A1A', fontSize: '0.95rem' }}>
                {selectedConv.type === 'pro_model' ? 'Seraphina L.' : 'Modeled Support'}
              </div>
          ...
          {/* Session context card (pro-model only) */}
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '0.5rem 0' }}>
            ...
            {isTyping && (
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0 1rem', margin: '0.25rem 0' }}>
                ...
                <div style={{
                  background: '#fff',
                  border: '1px solid rgba(74,42,26,0.1)',
                  ...
                  {[0, 150, 300].map(d => (
                    <span key={d} style={{
                      ...
                      background: '#8B1E3F',
                      opacity: 0.6,
                      animation: `typingDot 1.2s ${d}ms ease-in-out infinite`,
                    }} />
                  ))}
                </div>
              </div>
          ...
          {/* Quick Replies (pro-model channel only) */}
          ...
          <div style={{
            ...
            <textarea
              ...
              style={{
                ...
                borderRadius: '24px',
                border: '1.5px solid rgba(139,30,63,0.25)',
                fontSize: '0.88rem',
                fontFamily: '"Alike", serif',
                outline: 'none',
                resize: 'none',
