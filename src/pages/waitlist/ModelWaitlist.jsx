...
export default function ModelWaitlist() {
  ...
  useEffect(() => {
    const attrs = user?.signInDetails?.loginId ? { email: user.signInDetails.loginId } : {};
    const given = user?.attributes?.given_name || '';
    const family = user?.attributes?.family_name || '';
    setFormData((prev) => ({
      ...prev,
      email: prev.email || attrs.email || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        somethingFun: formData.whyInterested?.trim() || null,
        termsAccepted: true,
        termsAcceptedAt: now,
        status: 'pending',
      ...
      if (existing?.[0]) {
        ...
        await client.models.ModelProfile.create(payload);
      }
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.formContainer}>
        <h1 style={styles.title}>Model waitlist</h1>
        <p style={styles.subtitle}>
          You&apos;re signing up for early access. We&apos;ll email you when onboarding opens more broadly.
        </p>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={styles.formGroup}>
            <label style={styles.label}>What draws you to Modeled?</label>
            <textarea
              style={styles.textarea}
              value={formData.whyInterested}
              onChange={(ev) => setFormData({ ...formData, whyInterested: ev.target.value })}
